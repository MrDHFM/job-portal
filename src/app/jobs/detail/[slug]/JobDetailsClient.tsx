...
export default function JobDetailsClient({ job, company, category, similarJobs }: JobDetailsClientProps) {
  ...
  const handleApplyClick = () => {
    if (job.applicationMethod === "EXTERNAL_URL") {
      if (job.applicationUrl) {
        ...
        fetch(`/api/v1/jobs/${job.slug}/apply`, { method: "POST", body: JSON.stringify({ isInternal: false }) });
      }
    } else if (job.applicationMethod === "EMAIL") {
      if (job.recruiterEmail) {
        const subject = encodeURIComponent(`Application for ${job.title} - CareerDiscover`);
        const body = encodeURIComponent(`Hello Hiring Team,\n\nI am interested in applying for the ${job.title} position published on CareerDiscover.\n\nPlease find my resume attached.\n\nWarm regards.`);
  ...
  const handleApplySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!applyForm.name || !applyForm.email || !applyForm.phone || !applyForm.resumeUrl) {
      setApplyError("Please fill in name, email, phone, and standard Resume URL.");
      return;
    }
    ...
    try {
      ...
      setApplyError(e.message || "An unexpected error occurred during submission.");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 sm:p-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-4 border-y border-neutral-100 dark:border-neutral-850 text-sm mb-6">
          <div>
            <span className="block text-xs font-bold uppercase text-neutral-400">Work Mode</span>
            <span className="font-semibold text-neutral-800 dark:text-neutral-200">{job.workMode}</span>
          </div>
          <div>
            <span className="block text-xs font-bold uppercase text-neutral-400">Employment Type</span>
            <span className="font-semibold text-neutral-800 dark:text-neutral-200">{job.employmentType}</span>
          </div>
          <div>
            <span className="block text-xs font-bold uppercase text-neutral-400">Experience Level</span>
            <span className="font-semibold text-neutral-800 dark:text-neutral-200">{job.experienceLevel}</span>
          </div>
          <div>
            <span className="block text-xs font-bold uppercase text-neutral-400">Offered Salary</span>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <span className="text-xs text-neutral-400">
            {job.applicationMethod === "EXTERNAL_URL" && "🔒 Safe redirect to official company careers page."}
            {job.applicationMethod === "EMAIL" && "✉️ Click opens mail client with prefilled application details."}
            {job.applicationMethod === "INTERNAL" && "⚡ Apply directly via CareerDiscover's secure resume system."}
          </span>
        </div>
      ...
      {job.govOrganization && (
        <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            {job.govOrganization && <p><strong className="text-neutral-500">Department:</strong> {job.govOrganization}</p>}
            {job.govNotificationNumber && <p><strong className="text-neutral-500">Ref / Notification No:</strong> {job.govNotificationNumber}</p>}
            {job.govAgeLimit && <p><strong className="text-neutral-500">Age Limit:</strong> {job.govAgeLimit}</p>}
            {job.govApplicationFee && <p><strong className="text-neutral-500">Application Fee:</strong> {job.govApplicationFee}</p>}
            {job.govSelectionProcess && <p className="sm:col-span-2"><strong className="text-neutral-500">Selection Criteria:</strong> {job.govSelectionProcess}</p>}
          </div>
            ...
            {job.walkinDate && (
              <p>
                ...
                <span className="font-bold text-amber-700 dark:text-amber-400">
                  {new Date(job.walkinDate).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
                </span>
              </p>
            )}
            {job.walkinStartTime && (
              <p>
                <strong className="text-neutral-500">Timings:</strong> {job.walkinStartTime} {job.walkinEndTime ? `to ${job.walkinEndTime}` : ""}
              </p>
            )}
            {job.walkinContactInfo && <p className="sm:col-span-2"><strong className="text-neutral-500">Contact Information:</strong> {job.walkinContactInfo}</p>}
            {job.walkinVenue && <p className="sm:col-span-2"><strong className="text-neutral-500">Interview Venue:</strong> {job.walkinVenue}</p>}
            {job.walkinDocuments && <p className="sm:col-span-2"><strong className="text-neutral-500">Documents Required:</strong> {job.walkinDocuments}</p>}
            {job.walkinInstructions && <p className="sm:col-span-2"><strong className="text-neutral-500">Candidate Instructions:</strong> {job.walkinInstructions}</p>}
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 sm:p-8 space-y-6">
        ...
        {job.summary && (
          <div>
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Role Summary</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.summary}</p>
          </div>
        )}

        {job.aboutRole && (
          <div>
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">About the Team</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.aboutRole}</p>
          </div>
        )}

        <div>
          <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Detailed Job Description</h3>
          <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.description}</p>
        </div>

        {job.responsibilities && (
          <div>
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Responsibilities & Scope</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.responsibilities}</p>
          </div>
        )}

        {job.eligibility && (
          <div>
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Eligibility & Qualifications</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.eligibility}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t">
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-neutral-400 mb-2">Required Core Skills</h4>
            {job.requiredSkills ? (
              <div className="flex flex-wrap gap-1.5">
                {job.requiredSkills.split(",").map((s: string, i: number) => (
                  <span key={i} className="text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 text-neutral-850 dark:text-neutral-200 px-3 py-1.5 rounded-lg border border-neutral-200 dark:border-neutral-700">
                    {s.trim()}
                  </span>
                ))}
              </div>
          ...
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-neutral-400 mb-2">Preferred Tech Stack</h4>
            {job.preferredSkills ? (
              <div className="flex flex-wrap gap-1.5">
                {job.preferredSkills.split(",").map((s: string, i: number) => (
                  <span key={i} className="text-xs font-semibold bg-blue-50/50 dark:bg-neutral-800 text-blue-700 dark:text-blue-300 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-neutral-700">
                    {s.trim()}
                  </span>
                ))}
              </div>
        ...
        {job.benefits && (
          <div className="pt-4 border-t">
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">Employee Benefits</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-line">{job.benefits}</p>
          </div>
        )}

        {job.hiringProcess && (
          <div className="pt-4 border-t">
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-3">Recruitment Stages</h3>
            <div className="flex flex-wrap items-center gap-3">
              {job.hiringProcess.split("→").map((stage: string, idx: number) => (
      ...
      <div>
        ...
        {similarJobs.length === 0 ? (
          <p className="text-xs text-neutral-500 italic bg-white dark:bg-neutral-900 border p-4 rounded-xl text-center">
            No similar jobs available at the moment. All listings come from genuine entries.
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {similarJobs.map((item) => (
              ...
              >
                ...
                <p className="text-xs font-semibold text-blue-600 mt-1">{item.companyName}</p>
                <p className="text-[11px] text-neutral-400 mt-0.5">{item.city}, {item.country}</p>
                ...
                <div className="mt-4 pt-3 border-t flex items-center justify-between text-[11px] text-neutral-500">
                  ...
                  <span className="font-bold text-neutral-800 dark:text-neutral-200">
                    {item.isSalaryVisible && item.minSalary ? `$${(item.minSalary/1000).toFixed(0)}k+` : "Undisclosed"}
                  </span>
                </div>
              </Link>
            ))}
      ...
      {applyModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl max-w-lg w-full border border-neutral-200 dark:border-neutral-850 p-6 shadow-xl relative animate-in zoom-in-95 duration-200">
            ...
            {/* Close button */}
            <button
              onClick={() => { setApplyModalOpen(false); setApplySuccess(false); setApplyError(null); }}
            ...
            {applySuccess ? (
              <div className="text-center py-8 space-y-4">
                ...
                <h3 className="text-xl font-extrabold text-neutral-900 dark:text-white">Submission Succeeded!</h3>
                <p className="text-sm text-neutral-500 dark:text-neutral-400 max-w-sm mx-auto">
                  Your application and resume details have been safely logged in CareerDiscover's backend database. Recruiter personnel will contact you soon.
                </p>
                <button
                  onClick={() => { setApplyModalOpen(false); setApplySuccess(false); }}
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-6">
                  Submit your candidate application. Authorized hiring recruiters will inspect your profile directly from the PostgreSQL board.
                </p>
                ...
                <form onSubmit={applyFormSubmitCheck} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-neutral-400 mb-1">Your Full Name *</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-neutral-400 mb-1">Resume Document Link (PDF) *</label>
                      <div className="relative">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-neutral-400 mb-1">LinkedIn Profile</label>
                      <input
                        type="url"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-neutral-400 mb-1">Portfolio or GitHub Link</label>
                      <input
                        type="url"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase text-neutral-400 mb-1">Short Cover Letter</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="flex justify-end gap-2 pt-4 border-t">
                    ...
                    >
                      {applyLoading ? "Submitting..." : <>Submit Resume <Send className="h-3 w-3" /></>}
                    </button>
                  </div>
                </form>
              </div>
