...
import { FileText, Eye, Trash2, Mail, Phone, ExternalLink, SlidersHorizontal, CheckSquare } from "lucide-react";
...
export default function AdminApplicationsPage() {
  ...
  const filteredApps = apps.filter((app) => statusFilter === "" || app.status === statusFilter);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-neutral-900 border p-6 rounded-2xl shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-black text-neutral-900 dark:text-white flex items-center gap-2">
            <FileText className="h-6 w-6 text-blue-600" /> Candidate Applications
          </h1>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
            Audit standard candidate resume links submitted internally for published positions.
          </p>
        </div>
      ...
      {loading ? (
        <div className="bg-white p-12 text-center rounded-2xl animate-pulse">Loading candidates database...</div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-dashed rounded-2xl p-16 text-center">
          ...
          <h3 className="text-lg font-bold text-neutral-900 dark:text-white">No applications available yet</h3>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {filteredApps.map((app) => (
                <tr key={app.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-850/50 transition-colors">
                  <td className="p-4 pl-6">
                    <span className="font-extrabold text-neutral-850 dark:text-neutral-200 block">{app.name}</span>
                    <span className="text-xs text-neutral-400">Received: {new Date(app.createdAt).toLocaleDateString()}</span>
                  </td>
                  <td className="p-4 text-xs font-semibold text-neutral-600 dark:text-neutral-400 space-y-1">
                    <p className="flex items-center gap-1"><Mail className="h-3.5 w-3.5" /> {app.email}</p>
                    <p className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" /> {app.phone}</p>
                  </td>
                  <td className="p-4">
                    <span className="font-extrabold text-neutral-850 dark:text-neutral-200 block truncate max-w-xs">{app.job?.title}</span>
                    <span className="text-xs text-blue-600 dark:text-blue-400 font-bold">{app.company?.name}</span>
                  </td>
                  <td className="p-4">
                    <a
                      href={app.resumeUrl}
